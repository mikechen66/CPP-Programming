...
namespace construction {
    ...
    bool phone_number::operator<(const phone_number& x) const {
        if (m_country_code != x.m_country_code) 
            return m_country_code < x.m_country_code;
        if (m_area_code != x.m_area_code) 
            return m_area_code < x.m_area_code;
        if (m_number != x.m_number) 
            return m_number < x.m_number;
        if (m_extension != x.m_extension) 
            return m_extension < x.m_extension;
        return false;
    }
    ...
    phone_number_container<Tcontainer>::get_lower_bound(unsigned short m_country_code, unsigned short m_area_code) {
        iterator i = c.begin();
        while (i != c.end()) {
            if (i->country_code() == m_country_code && i->area_code() == m_area_code) 
                break;
            i++;
        }
        return i;
    ...
    phone_number_container<Tcontainer>::get_upper_bound(unsigned short m_country_code, unsigned short m_area_code) {
        ...
        while (i != c.end()) {
            if (i->CountryCode() != m_country_code || i->AreaCode() != m_area_code) 
                break;
            i++;
        }
        return i;
    ...
    phone_number_container<Tcontainer>::get_equal_bound(unsigned short m_country_code, unsigned short m_area_code) {
        ...
        iterator i2 = i1;
        while (i2 != c.end()) {
            if(i2->country_code() != m_country_code || i2->area_code() != m_area_code) 
                break;
            i2++;
        }
...
$g++ -o main *.cpp
$main
printing all 206 numbers:
+1 (206) 5551111
+1 (206) 5551112
+1 (206) 5551113
printing all 360 numbers:
+1 (360) 5551111
printing all 425 numbers:
+1 (425) 5551111
+1 (425) 5551112 x1234
 */