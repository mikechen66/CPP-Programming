...
namespace construction {
    class Date {
        ...
        void read_full_year(std::istream& input) { 
            if (!(input >> y)) 
                throw FormatError(); 
        }
        ...
        void read_month_number(std::istream& input) { 
            if (!(input >> m)) 
                throw FormatError(); 
        }

        void read_day(std::istream& input) { 
            if (!(input >> d)) 
                throw FormatError(); 
        }
...
$g++ -o main *.cpp
$main
enter some dates (empty line to finish):
 */