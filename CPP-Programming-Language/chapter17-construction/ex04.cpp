        ...
        if (cmd.get(ch)) {
            ...
            switch (ch) {
                ...
                case 'm':
                {
                    ...
                    int col2;
                    unsigned int n;
                    if (!get_col(cmd, col1) || !get_col(cmd, col2)){
                        cout << "invalid column" << endl;
                        break;
                    }
                ...
                case 'x':
                {
                    ...
                    gameover = true;
                    for (int i = 0; i < 4; i++)
                        if (foundation[i].empty() || foundation[i].back()->number() != 13) 
                            gameover = false;
                ...
                case 'f':
                {
                    ...
                    int col;
                    if (!get_col(cmd, col) || tableau[col].empty() || tableau[col].shown() != 0) {
                        cout << "invalid column" << endl;
                        break;
                    }
...
type a command:
  d                       draw from stock
  t <col>                 move top drawn card to column
  m <col1> <col2> [num]   move num (default 1) cards from col1 to col2
  f <col>                 flip face-down card in col
  x <fs> <col>            move bottom card in col to foundation stack fs
  x <fs>                  move top card from waste to foundation stack fs
Stock: [21]
Waste: 1D 9C 1S <-
Foundations: [  ] [  ] [  ] [  ] 
 1  2  3  4  5  6  7 
