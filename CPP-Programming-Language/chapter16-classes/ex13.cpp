...
namespace classes {
    ...
    class Vector_classes_2 : public Container {
        static const int DefaultCapacity = 16;
        size_t sz;
        size_t capacity;
        Object** elements;
    public:
        explicit Vector_classes_2() : sz(0), capacity(DefaultCapacity), elements(new Object*[DefaultCapacity]) {}
        ~Vector_classes_2() { delete[] elements; }
    ...
    void Vector_classes_2::put(Object* o) {
        if (sz == capacity) {
            capacity *= 2;
    ...
    template<class T> class Vector_classes_1 {
        ...
        size_t sz;
    public:
        explicit Vector_classes_1(size_t n) : sz(n), elements(new T[n]) {}
        ~Vector_classes_1() { delete[] elements; }
        T& operator[](size_t i) { return elements[i]; }
        size_t size() { return sz; }
    };

    template<class T> class Vector_classes_1_wrapper {
        Vector_classes_1<T>& v;
    public:
        typedef T* iterator;

        Vector_classes_1_wrapper(Vector_classes_1<T>& v) : v(v) {}
        iterator begin() { return &v[0]; }
        iterator end() { return &v[v.size()]; }     
    };

    template<class T> class Vector_classes_2_wrapper {
        Vector_classes_2& v;
    public:
        ...
        class Itor {
            ...
            int i;
        public:
            Itor(Vector_classes_2& v, int i) : v(v), i(i) {}
        ...
        Vector_classes_2_wrapper(Vector_classes_2& v) : v(v) {}
...
int main() {
    ...
    Vector_classes_1<int> v(3);
    v[0] = 1;
    v[1] = 2;
    v[2] = 3;
    Vector_classes_1_wrapper<int> vw(v);

    for(Vector_classes_1_wrapper<int>::iterator i = vw.begin(); i != vw.end(); i++) {
        cout << *i << endl;
    }

    Vector_classes_2 v2;
    v2.put(new Vector_classes_2_wrapper<int>::Holder(1));
    v2.put(new Vector_classes_2_wrapper<int>::Holder(2));
    v2.put(new Vector_classes_2_wrapper<int>::Holder(3));
    Vector_classes_2_wrapper<int> vw2(v2);

    Vector_classes_2_wrapper<int>::iterator end = vw2.end();
    for (Vector_classes_2_wrapper<int>::iterator i = vw2.begin(); i != end; i++) {
        cout << *i << endl;
    }
