        ...
        default:
            if (isalpha(ch)) {
                ...
                while (input->get(ch) && isalnum(ch)) 
                    curr_sym.string_value.push_back(ch);
                input->putback(ch);
    ...
    switch (curr_sym.curr_tok) {
        ...
        case Lexer::NAME:
            ...
            if (get_token(input) == Lexer::LP) {
                ...
                double& v = table[name];
                if (curr_sym.curr_tok == Lexer::ASSIGN) 
                    v = expr(input, true);
                return v;
            }
            }
...
int main(int argc, char* argv[]) {
    ...
    while (*input) {
        try {
            get_token(input);
            if (curr_sym.curr_tok == Lexer::END) 
                break;
            if (curr_sym.curr_tok == Lexer::PRINT) 
                continue;
