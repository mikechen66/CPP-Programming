...
public:
    ...
    void addYear() {
        ...
        if(m_month == 2) {
            if(m_day == 29) {
                m_day = 28;
            } else if (m_day == 28 && isLeapYear(m_year)) {
                m_day = 29;
            }
        }
    }
    ...
    int dayOfWeek() {
        ...
        int x = m_year - 1900;
        x += x / 4;
        if(isLeapYear(m_year) && m_month < 3) 
            x -= 1;
        x += m_day;
        if(m_month==5) 
            x += 1;
        if(m_month==8) 
            x += 2;
        if(m_month==2 || m_month==3 || m_month==11) 
            x += 3;
        if(m_month==6) 
            x += 4;
        if(m_month==9 || m_month==12) 
            x += 5;
        if(m_month==4 || m_month==7) 
            x += 6;
        return x % 7;
    }
...
$g++ -o main *.cpp
$main
Adding one year: 1-1-32745
Adding one month: 0-2-29
Adding one day: 0-2-1
Day of week: Wed
Next monday: 0-2-5
 */