...
/* Floyd Algorithm 
 * the shortest distance (SD) between any two adjacency vertices
 */
...
void shortestPath(int vertex_total) {       
    int i, j, k;
    extern int distance[SIZE][SIZE];                   /* Declare the external variable */

    for (i=1; i<=vertex_total; i++ )                   /* Initialize the array */
        for (j=i; j<=vertex_total; j++) {
            distance[i][j]=Graph_Matrix[i][j]; 
            distance[j][i]=Graph_Matrix[i][j];        
        }

    for (k=1; k<=vertex_total; k++ )                   /* (Find the SD between any two adjacency vertices*/
