...
void shortestPath(int vertex1, int vertex_total) {    /* shortest path between a single point to all vertices */
    extern int distance[SIZE];                        /* Declare it as an external variable */
    int shortest_vertex = 1;         
    int shortest_distance;                            /* Record the shortest distance */
    ...
    for (i=1; i<=vertex_total-1; i++) {      
        ...
        for (j=1; j<=vertex_total; j++) {            /* Find out the vertice of shortest distance */
        ...
        for (j=1; j<=vertex_total; j++) {            /* Compute the shortest distance */
            if (goal[j] == 0 && 
...
int main(void) {
    extern int distance[SIZE];                       /* Declare the external variable */
